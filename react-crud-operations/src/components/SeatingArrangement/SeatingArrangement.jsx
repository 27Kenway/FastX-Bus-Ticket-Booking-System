  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`https://localhost:7114/api/BusSeats/${busId}?deptId=${deptdate}`, {
          headers: {
          ...
          <div className="seats">
            {seats &&
              seats.map((seat) => (
                <div
                  key={seat.seatNo}
                  className={`seat ${seat.isBooked ? 'unavailable' : 'available'}`}
